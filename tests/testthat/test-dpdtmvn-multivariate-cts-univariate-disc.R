...
test_that("dpdtmvn works -- 1 continuous, 1 discrete, no truncation, sigma, upper integration limit = point", {
	...
	dpdtmvn_result_log <- dpdtmvn(x,
		...
		continuous_vars = 1,
		discrete_vars = 2,
		discrete_var_range_fns = list(list(a = "floor_x_minus_1", b = "floor", in_range = "equals_integer")),
		log = TRUE,
		validate_level = 1)
...
test_that("dpdtmvn works -- 1 continuous, 1 discrete, no truncation, sigma, upper integration limit = point + 0.5", {
    ...
    dpdtmvn_result_log <- dpdtmvn(x,
        ...
        continuous_vars = 1,
        discrete_vars = 2,
        discrete_var_range_fns = list(list(a = x_minus_0.5, b = x_plus_0.5, in_range = equals_integer)),
        log = TRUE,
        validate_level = 1)
